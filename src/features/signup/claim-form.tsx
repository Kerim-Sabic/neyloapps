...
export function ClaimForm({handle,setHandle,onHeld,onFocus,onComplete,campaign,ready,invitation,initialEmail='',source,campaignTag}:Props){
  ...
  useEffect(()=>{
    ...
    request('signup/pending',z.object({pending:pendingSchema.nullable()})).then(({pending})=>{
      if(!active||!pending||pending.finalized)return;setEmail(pending.email);setHandle(pending.handle);setAccepted(true);
  ...
  async function submit(event:React.FormEvent){
    ...
    try{
      ...
      const result=await request('signup/start',startResult,{handle,email,accepted,termsVersion:campaign?.version,invitation:invitation?.code,source,campaignTag,offerDisplayed:Boolean(campaign&&campaign.remaining>0)});
  ...
  return <form className="claim-form" onSubmit={submit} onFocusCapture={()=>onFocus(true)} onBlurCapture={e=>{if(!e.currentTarget.contains(e.relatedTarget))onFocus(false);}} aria-label="Reserve your NEYLO handle">
    ...
    <label className="checkbox-row"><input type="checkbox" checked={accepted} onChange={e=>setAccepted(e.target.checked)} required disabled={!ready}/><span>I agree to the <Link href="/terms" target="_blank">campaign terms</Link> and <Link href="/privacy" target="_blank">privacy notice</Link>{campaign?.region?`, and confirm I’m ${campaign.minimumAge} or older`:''}.</span></label></>}
    <p id="form-error" className="form-error" role="alert">{error}</p>
    <button className="button primary" type="submit" disabled={busy||!ready} aria-busy={busy}>{busy?'One moment…':phase.kind==='code'?'Verify & reserve my handle':phase.kind==='recover'?'Finish my reservation':ready?'Claim my @handle':'Reservations open soon'}{!busy&&<ArrowRight size={18} aria-hidden="true"/>}</button>
