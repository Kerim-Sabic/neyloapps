...
export function Landing({campaign,ready,invitation,initialEmail,source='direct',campaignTag='',support}:{campaign:Campaign|null;ready:boolean;invitation?:Invitation;initialEmail?:string;source?:string;campaignTag?:string;support:string|null}){
  const router=useRouter();const {credit}=useCreditDisplay();
  ...
  return <main id="main" className="landing"><section className={`hero ${focused?'form-focused':''}`}>
    ...
    <div className="hero-copy">{invitation&&<p className="invitation-note">An invitation from <strong>@{invitation.handle}</strong>{invitation.mayEarn&&<span>They may receive {credit(5000)} in launch fee credits when you complete an eligible signup.</span>}</p>}
      ...
      <p className="offer-line">{hasOffer?<>Get <strong>{credit(10000)}</strong> in NEYLO credits.</>:<>Your name is still yours to reserve.</>}</p>
    </div>
  ...
  <section className="referral-intro" aria-labelledby="referral-heading"><p className="eyebrow">BETTER WITH YOUR PEOPLE</p><h2 id="referral-heading">Invite a friend.<br/>Get <span>{credit(5000)}</span> more.</h2><p>After you reserve your handle, share your personal invitation. A friend’s eligible verified signup earns you another {credit(5000)} in launch fee credits.</p><p className="small-copy">For eligible founding accounts. First three qualifying friends.<br/>Welcome credit + three referral rewards.<br/><strong>{credit(25000)} maximum.</strong></p><a className="inline-link" href="#handle">Make a name for yourself <ArrowUpRight size={16}/></a></section>
