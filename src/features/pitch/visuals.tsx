...
export function RoutingProof(){
  ...
  return <div className="pitch-route-proof"><div className="pitch-proof-top"><span><span className="pitch-tiny-dot"/>ROUTING WORKBENCH</span><span>USDC → EUR</span></div>
    ...
    <div className="pitch-proof-route" aria-label={selected?.route.nodes.map(n=>n.name).join(' to ')??'No route'}>{['Wallet','Settlement','FX','EUR rail','Anna'].map((n,i)=><span key={n}><i>{i===4?<AtSign size={12}/>:i+1}</i><small>{n}</small>{i<4&&<b/>}</span>)}</div>
