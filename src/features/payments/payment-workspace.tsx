...
export function PaymentWorkspace() {
  ...
  const recipientReady = validName(form.name) && validBosnianIban(form.iban);
  ...
  function advance() {
    if (step === 0 && !recipientReady) {
      setError('Enter the recipient’s full name and a valid Bosnian IBAN. Ask them for the account details directly.'); return;
    }
    ...
    if (step === 2) {
      ...
      try {
        setPlan(createPlan(form, crypto.randomUUID(), new Date().toISOString()));
        setStatus('prepared'); setReportConfirm(false);
      } catch { setError('We could not prepare this payment. Check the details and try again.'); return; }
    }
    go(step + 1);
  }
  function reset() {
  ...
  return <MotionConfig reducedMotion="user"><main id="main" className="payment-workspace">
    ...
    <header className="pay-intro"><p className="pay-eyebrow">A LITTLE LESS FRICTION</p><h1>From you.<br/><span>To someone who matters.</span></h1><p>Prepare a payment in BAM. Make the transfer securely in your own bank app.</p></header>
    <div className="pay-layout">
        ...
        <AnimatePresence mode="wait" initial={false}>
          <motion.div className="pay-step" key={step} initial={{ opacity: 0, y: reducedMotion ? 0 : 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: reducedMotion ? 0 : -6 }} transition={{ duration: reducedMotion ? 0 : 0.2, ease: [0.22, 1, 0.36, 1] }} onAnimationComplete={focusStep}>
            <div className="pay-step-heading"><span className="pay-step-icon" aria-hidden="true">{step === 0 ? <UserRound size={23}/> : step === 1 ? <span>KM</span> : step === 2 ? <ShieldCheck size={23}/> : <Landmark size={23}/>}</span><p className="pay-eyebrow">STEP {step + 1} OF 4</p>
              ...
              <p>{['Use the account details your recipient gave you.', 'Small everyday payments, thoughtfully prepared.', 'Confirm the person, the account and the amount.', 'Your instructions are ready. No money has been sent by Neylo.'][step]}</p>
            </div>
            ...
            {step < 3 ? <form onSubmit={event => { event.preventDefault(); advance(); }} noValidate>
              {step === 0 && <div className="pay-fields">
                ...
                <p id="recipient-help" className="pay-field-help">We check the account format and checksum only. Neylo cannot verify who owns this account. @handle payments will become available when a payment provider is connected.</p>
              ...
              {step === 2 && <div className="pay-fields">
                ...
                <dl className="pay-details"><div><dt>Recipient, entered by you</dt><dd>{form.name}</dd></div><div><dt>IBAN</dt><dd className="pay-mono">{formattedIban(form.iban)}</dd></div>{form.note && <div><dt>Purpose</dt><dd>{form.note}</dd></div>}<div><dt>Neylo planning fee</dt><dd>Free</dd></div><div><dt>Bank fees & arrival</dt><dd>Check in your bank app</dd></div><div><dt>Recipient net amount</dt><dd>Not confirmed; bank fees may apply</dd></div></dl>
                <label className="pay-confirm"><input type="checkbox" checked={verifiedByUser} onChange={e => setVerifiedByUser(e.target.checked)}/><span>I checked the recipient and account details through a trusted channel. I understand that Neylo will not send this payment.</span></label>
              </div>}
              <div className="pay-actions">{step > 0 && <button type="button" className="pay-back" onClick={() => go(step - 1)}><ArrowLeft size={17} aria-hidden="true"/>Back</button>}<button type="submit" className="pay-primary">{step === 2 ? 'Prepare bank instructions' : step === 1 ? 'Review payment' : 'Continue'}<ArrowRight size={18} aria-hidden="true"/></button></div>
              ...
              <div className="pay-report" data-reported={status === 'reported_sent'}><FileText size={20} aria-hidden="true"/><div><strong>{status === 'reported_sent' ? 'You marked this as sent' : 'Prepared · not sent'}</strong><p>{status === 'reported_sent' ? 'This is your own record. Neylo has not confirmed the payment with your bank or the recipient.' : 'Open your bank app separately and review its fees before authorizing.'}</p></div></div>
              <dl className="pay-copy-details">
                {[['Recipient name', plan.recipient.name], ['IBAN', formattedIban(plan.recipient.iban)], ['Domestic account', plan.recipient.iban.slice(4)], ['Amount (BAM)', (plan.amountMinor / 100).toFixed(2)], ...(plan.note ? [['Purpose', plan.note]] : [])].map(([label = '', value = '']) => <div key={label}><dt>{label}</dt><dd><span>{value}</span><button type="button" aria-label={`Copy ${label.toLowerCase()}`} onClick={() => copy(label === 'IBAN' ? plan.recipient.iban : value, label)}>{copied === label ? <Check size={16} aria-hidden="true"/> : <Copy size={16} aria-hidden="true"/>}</button></dd></div>)}
              </dl>
              ...
              <button type="button" className="pay-secondary pay-full" onClick={download}><Download size={17} aria-hidden="true"/>Download payment plan</button>
              <p className="pay-action-help">Contains bank details. Not a payment receipt.</p>
              {status === 'prepared' ? <div className="pay-report-action"><label className="pay-confirm"><input type="checkbox" checked={reportConfirm} onChange={e => setReportConfirm(e.target.checked)}/><span>I have already authorized this payment in my bank app.</span></label><button type="button" className="pay-secondary pay-full" disabled={!reportConfirm} onClick={() => { setStatus(reportSent(status)); setNotice('Marked as sent by you. Bank confirmation is unavailable.'); }}><CheckCheck size={17} aria-hidden="true"/>Mark as sent by me</button></div> : <button type="button" className="pay-text" onClick={() => { setStatus('prepared'); setReportConfirm(false); setNotice('Your personal status was reset. This does not cancel or reverse a bank payment.'); }}>Undo my status · does not cancel a transfer</button>}
      ...
      <aside className="pay-aside" aria-label="Payment summary">
        <div className="pay-summary"><div className="pay-summary-top"><span className="pay-eyebrow">YOUR PAYMENT PLAN</span><span className="pay-outline-icon"><Landmark size={18} aria-hidden="true"/></span></div>
          <div className="pay-person"><span className="pay-avatar" aria-hidden="true">{form.name.trim().slice(0, 1).toUpperCase() || <UserRound size={26}/>}</span><div><strong>{form.name.trim() || 'Someone who matters'}</strong><p>{form.iban ? `Account ending ${form.iban.replace(/\s/g, '').slice(-4)}` : 'Their bank. Their money.'}</p></div></div>
        ...
        <div className="pay-assurance"><LockKeyhole size={18} aria-hidden="true"/><div><h3>Private by design.</h3><p>No bank login. No card details. No money held by Neylo. Your plan is kept only in this page’s memory.</p></div></div>
