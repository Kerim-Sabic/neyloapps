...
import { createPlan, formattedIban, instructions, reportSent, validName, validNote, type PaymentPlan, type PlanStatus } from './domain';
import './payments.css';
import { z } from 'zod';
import { request, RequestError } from '@/lib/client';
import { BankCountryField, CurrencyField, UnavailableFormat } from './route-fields';
import { bankFormats, currencies, decimalMoney, formatMoney, isBankCountry, planAmount, suggestedCurrency, validInternationalIban, type Currency } from './international';
...
const initialForm = { name: '', iban: '', amount: '', note: '', bankCountry:'BA', currency:'BAM' as Currency };
...
export function PaymentWorkspace({preview=false}:{preview?:boolean}) {
  ...
  const amount = planAmount(form.amount,form.currency);
  const recipientReady = mode==='handle' ? Boolean(recipient) : validName(form.name) && validInternationalIban(form.iban,form.bankCountry);
  ...
  async function advance() {
    ...
    if (step === 0 && !recipientReady) {
      setError(mode==='handle'?'Find and select a recipient by username first.':'Enter the recipient’s full name and a valid IBAN for the selected bank country. Ask them for the account details directly.'); return;
    }
    if (step === 1 && (amount === null || !validNote(form.note))) {
      setError(`Enter an amount from 1 to 100 ${form.currency}, with at most ${currencies[form.currency].digits} decimal places, and a purpose of up to 100 characters.`); return;
    }
    if (step === 2) {
      ...
      try {
        ...
        if(mode==='handle'&&recipient){
          const result=preview?{destination:{accountName:recipient.accountName,iban:bankFormats[recipient.bankCountry].example,bankName:recipient.bankName,bic:'',bankCountry:recipient.bankCountry,currency:recipient.currency}}:await request('payments/recipient/prepare',z.object({destination:destinationSchema}),{handle:recipient.handle,version:recipient.version});
          if(result.destination.currency!==recipient.currency||result.destination.bankCountry!==recipient.bankCountry)throw new Error('Destination changed');
          input={...form,name:result.destination.accountName,iban:result.destination.iban,currency:result.destination.currency,bankCountry:result.destination.bankCountry};
          bankName=result.destination.bankName;bic=result.destination.bic;
        }
        setPlan({...createPlan({...input,bankCountry:isBankCountry(input.bankCountry)?input.bankCountry:undefined}, crypto.randomUUID(), new Date().toISOString()),...(recipient&&mode==='handle'?{recipientHandle:recipient.handle,bankName,bic}:{})});
  ...
  return <MotionConfig reducedMotion="user"><main id="main" className="payment-workspace">
    {preview&&<p className="pay-error" role="note">DESIGN PREVIEW · Examples: @nadin · BAM, @alex · EUR, @maya · GBP. Illustrative bank details only. Do not send money to these details. No account is connected.</p>}
    ...
    <div className="pay-layout">
      <section className="pay-card" aria-label="Prepare a bank payment">
        ...
        <AnimatePresence mode="wait" initial={false}>
          <motion.div className="pay-step" key={step} initial={{ opacity: 0, y: reducedMotion ? 0 : 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: reducedMotion ? 0 : -6 }} transition={{ duration: reducedMotion ? 0 : 0.2, ease: [0.22, 1, 0.36, 1] }} onAnimationComplete={focusStep}>
            <div className="pay-step-heading"><span className="pay-step-icon" aria-hidden="true">{step === 0 ? <UserRound size={23}/> : step === 1 ? <span>{form.currency}</span> : step === 2 ? <ShieldCheck size={23}/> : <Landmark size={23}/>}</span><p className="pay-eyebrow">STEP {step + 1} OF 4</p>
            ...
            {step < 3 ? <form onSubmit={event => { event.preventDefault(); advance(); }} noValidate>
              {step===0&&mode==='handle'&&<HandlePicker preview={preview} selected={recipient} onSelect={value=>{setRecipient(value);setForm({...form,name:value?.accountName??'',iban:'',amount:'',currency:value?.currency??'BAM',bankCountry:value?.bankCountry??'BA'});setVerifiedByUser(false);}}/>}
              {step === 0 && mode==='manual' && <div className="pay-fields">
                <BankCountryField value={form.bankCountry} onChange={bankCountry=>{setForm({...form,bankCountry,iban:'',currency:isBankCountry(bankCountry)?suggestedCurrency(bankCountry):'BAM',amount:''});setVerifiedByUser(false);}}/>
                {!isBankCountry(form.bankCountry)&&<UnavailableFormat/>}
                <CurrencyField value={form.currency} onChange={currency=>{setForm({...form,currency,amount:''});setVerifiedByUser(false);}}/>
                <p className="pay-field-help">Use the currency the recipient says this account accepts. Neylo does not convert currencies.</p>
                ...
                <label htmlFor="pay-iban">{form.bankCountry==='BA'?'Bosnian IBAN':'IBAN'}<input id="pay-iban" className="pay-iban-input" autoComplete="off" spellCheck={false} autoCapitalize="characters" maxLength={50} value={form.iban} placeholder="As supplied by the recipient" onChange={e => { setForm({ ...form, iban: e.target.value }); setVerifiedByUser(false); }} aria-describedby="recipient-help"/></label>
              ...
              {step === 1 && <div className="pay-fields">
                <label htmlFor="pay-amount">Payment amount<div className="pay-amount-field"><input id="pay-amount" inputMode="decimal" autoComplete="off" maxLength={9} value={form.amount} placeholder="0.00" onChange={e => setForm({ ...form, amount: e.target.value })} aria-describedby="amount-help"/><span>{form.currency}</span></div></label>
                <div className="pay-presets" aria-label="Suggested amounts">{[10, 25, 50, 100].map(value => <button key={value} type="button" aria-pressed={amount === value * 10**currencies[form.currency].digits} onClick={() => setForm({ ...form, amount: String(value) })}>{value} {form.currency}</button>)}</div>
                <p id="amount-help" className="pay-field-help">Prepare 1–100 {form.currency}. This is the payment amount, before any fees your bank may charge.</p>
              ...
              {step === 2 && <div className="pay-fields">
                <div className="pay-review-amount">{amount !== null ? formatMoney(amount,form.currency) : 'Check amount'}</div>
                <dl className="pay-details"><div><dt>{recipient?'Recipient':'Recipient, entered by you'}</dt><dd>{recipient?`@${recipient.handle}`:form.name}</dd></div>{!recipient&&<div><dt>IBAN</dt><dd className="pay-mono">{formattedIban(form.iban)}</dd></div>}{form.note && <div><dt>Purpose</dt><dd>{form.note}</dd></div>}<div><dt>Neylo planning fee</dt><dd>Free</dd></div><div><dt>Bank fees & arrival</dt><dd>Check in your bank app</dd></div><div><dt>Currency conversion</dt><dd>No FX quote · confirm with your bank</dd></div><div><dt>Recipient net amount</dt><dd>Not confirmed; bank fees may apply</dd></div></dl>
              ...
              <dl className="pay-copy-details">
                {[['Recipient name', plan.recipient.name], ...(plan.bankName?[['Bank',plan.bankName]]:[]),['IBAN', formattedIban(plan.recipient.iban)], ...(plan.bankCountry==='BA'&&plan.currency==='BAM'?[['Domestic account',plan.recipient.iban.slice(4)]]:[]), ...(plan.bic?[['SWIFT / BIC',plan.bic]]:[]),[`Amount (${plan.currency})`, decimalMoney(plan.amountMinor,plan.currency)], ...(plan.note ? [['Purpose', plan.note]] : [])].map(([label = '', value = '']) => <div key={label}><dt>{label}</dt><dd><span>{value}</span><button type="button" aria-label={`Copy ${label.toLowerCase()}`} onClick={() => copy(label === 'IBAN' ? plan.recipient.iban : value, label)}>{copied === label ? <Check size={16} aria-hidden="true"/> : <Copy size={16} aria-hidden="true"/>}</button></dd></div>)}
              </dl>
              <p className="pay-field-help">Confirm the account currency, additional beneficiary fields, intermediary fees and any FX rate with your bank. Neylo provides no conversion quote. This is not an instant-payment guarantee.</p>
      ...
      <aside className="pay-aside" aria-label="Payment summary">
        <div className="pay-summary"><div className="pay-summary-top"><span className="pay-eyebrow">YOUR PAYMENT PLAN</span><span className="pay-outline-icon"><Landmark size={18} aria-hidden="true"/></span></div>
          ...
          <p className="pay-summary-amount">{amount !== null ? formatMoney(amount,form.currency) : `— ${form.currency}`}</p><p className="pay-summary-caption">{status === 'reported_sent' ? 'Reported sent · not bank-confirmed' : 'Prepared here. Authorized in your bank.'}</p>
    ...
    <div className="pay-bottom"><span>STARTING IN BOSNIA · BUILT TO EXPAND</span><span>Thoughtful payments start with clear details.</span></div>
  </main></MotionConfig>;
}
