...
const empty = { accountName:'',iban:'',bankName:'',bic:'',discoverable:false };
export function ReceivingSettings({handle}:{handle:string}) {
  ...
  return <main id="main" className="payment-workspace receiving-page"><Link className="pay-support" href="/account"><ArrowLeft size={16}/>Your account</Link><header className="pay-intro"><p className="pay-eyebrow">YOUR NAME IS THE ADDRESS</p><h1>Your bank.<br/><span>Behind @{handle}.</span></h1><p>Add your receiving details once. Friends look you up by username.</p></header>
    <section className="pay-card pay-step"><div className="pay-step-heading"><span className="pay-step-icon"><Landmark size={22}/></span><h2>Where should money arrive?</h2><p>One personal BAM account in Bosnia and Herzegovina. No bank login or card information.</p></div>
      ...
      {!loading&&<form onSubmit={e=>{e.preventDefault();void save();}}><fieldset disabled={busy||!ready} className="pay-settings-fields pay-fields">
        ...
        <label>Bank<select value={bankChoices.includes(data.bankName as typeof bankChoices[number])?data.bankName:data.bankName?'Other bank':''} onChange={e=>setData({...data,bankName:e.target.value})}><option value="">Choose your bank</option>{bankChoices.map(bank=><option key={bank}>{bank}</option>)}</select></label>
        {(data.bankName==='Other bank'||data.bankName&&!bankChoices.includes(data.bankName as typeof bankChoices[number]))&&<label>Bank’s legal name<input value={data.bankName==='Other bank'?'':data.bankName} maxLength={100} onChange={e=>setData({...data,bankName:e.target.value||'Other bank'})}/></label>}
        <label>Bosnian IBAN<input value={data.iban} autoComplete="off" spellCheck={false} maxLength={30} placeholder="BA39 ···· ···· ···· ····" onChange={e=>setData({...data,iban:e.target.value})}/></label>
        <details className="pay-faq"><summary>Additional bank details</summary><label>SWIFT / BIC <span className="pay-optional">Optional for this domestic route</span><input value={data.bic} autoComplete="off" maxLength={11} placeholder="8 or 11 characters" onChange={e=>setData({...data,bic:e.target.value.toUpperCase()})}/></label><p>Copy this from your bank if provided. International transfers and other currencies are not available; we do not collect unrelated address or intermediary-bank details.</p></details>
        ...
        <button className="pay-primary pay-full" type="submit">{busy?'Saving…':saved?'Update receiving account':`Link to @${handle}`}</button>
      </fieldset></form>}
