...
export function DemoApp() {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      const target = area.querySelector(phase === 'completed' ? '.demo-receive' : '.demo-journey');
      if (target) area.scrollTo({ top: Math.max(0, target.getBoundingClientRect().top - area.getBoundingClientRect().top + area.scrollTop - 16), behavior: reduced ? 'instant' : 'smooth' });
    });
  ...
  return <MotionConfig reducedMotion={reduced ? 'always' : 'user'} transition={{ duration: reduced ? 0 : 0.4, ease: [0.22, 1, 0.36, 1] }}>
    <main id="main" className="neylo-demo" data-neylo-demo data-reduced={reduced}>
      <div className="demo-stage-brand"><Link href="/" aria-label="NEYLO home">neylo</Link><span>ONE ADDRESS. EVERY JOURNEY.</span></div>
      <div className="demo-stage">
        ...
        <div className="demo-device"><div className="demo-device-edge" aria-hidden="true"/><div className="demo-phone">
          <header className="demo-app-header"><button className="demo-wordmark" onClick={() => { app.reset(); setSearch(''); }} aria-label="NEYLO app home">neylo</button><span className="demo-profile"><span>N</span>@noomy</span></header>
          <div className="demo-app-scroll" ref={scrollArea}>
            ...
            <motion.section layout="position" className="demo-composer" aria-label="Transfer composer" data-phase={phase}>
              {!person ? <><label className="demo-search"><Search size={19}/><input aria-label="Search people" placeholder="Search a name or @handle" value={search} onChange={event => { manual(); setSearch(event.target.value); }}/></label><div className="demo-section-label">YOUR PEOPLE <span>02</span></div><div className="demo-contacts">{CONTACTS.filter(item => `${item.handle} ${item.name} ${item.bank} ${item.country}`.toLowerCase().includes(search.toLowerCase())).map(item => <button className="demo-contact" key={item.id} onClick={() => action({ type: 'SELECT', recipient: item.id })}><motion.span layoutId={`person-${item.id}`} className={`demo-avatar is-${item.id}`}>{item.initials}</motion.span><span><strong>{item.handle}</strong><small>{item.bank} · {item.country}</small></span><ChevronRight size={19}/></button>)}{!CONTACTS.some(item => `${item.handle} ${item.name} ${item.bank} ${item.country}`.toLowerCase().includes(search.toLowerCase())) && <p className="demo-empty">No matches. Try Anna or Kesh.</p>}</div><HomeSources/><Link className="demo-pair-entry" href="/demo/nadin"><span><strong>Two people. One arrival.</strong><small>Open the connected experience</small></span><ArrowUpRight size={20}/></Link><RecentReceipts app={app}/></> : <>
                <motion.div layout="position" className="demo-selected-person"><motion.span layoutId={`person-${person.id}`} className={`demo-avatar is-${person.id}`}>{person.initials}</motion.span><div><span>Sending to</span><strong>{person.handle}</strong><small>{person.bank} · {person.country}</small></div>{isEditing && <button className="demo-icon-button" onClick={() => action({ type: 'RESET' })} aria-label="Change recipient"><X size={18}/></button>}</motion.div>
                ...
                {phase === 'completed' && transfer && <motion.div className="demo-receipt" initial={reduced?false:{opacity:0,y:8}} animate={{opacity:1,y:0}}><dl><div><dt>You sent</dt><dd>{money(transfer.quote.sentMinor, transfer.quote.sourceCurrency)}</dd></div><div><dt>Fee included</dt><dd>{money(transfer.quote.feeMinor, transfer.quote.sourceCurrency)}</dd></div><div><dt>Source</dt><dd>{person.source}</dd></div><div><dt>Route</dt><dd>{transfer.quote.route.name}</dd></div><div><dt>Reference</dt><dd className="demo-reference">NEY-{transfer.id.toUpperCase()}</dd></div></dl><button className="demo-text-button" onClick={copyReference}>{copied ? <Check size={16}/> : <Copy size={16}/>}<span aria-live="polite">{copied ? 'Reference copied' : 'Copy reference'}</span></button></motion.div>}
              </>}
            </motion.section>
