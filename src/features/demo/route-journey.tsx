...
export function RouteJourney({ nodes, transfer, expanded, onToggle, reduced }: Props) {
  ...
  const connectors = useRef<(SVGGElement | null)[]>([]);
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      if (first && last) {
        ...
        nodes.slice(0, -1).forEach((_, index) => {
          const from = circles[index], to = circles[index + 1];
          const parts = nodes.length - 1;
          const line = unfolding && from && to ? connectorGeometry(from, to) : overall ? {
            x1: overall.x1 + (overall.x2 - overall.x1) * index / parts, y1: overall.y1 + (overall.y2 - overall.y1) * index / parts,
            x2: overall.x1 + (overall.x2 - overall.x1) * (index + 1) / parts, y2: overall.y1 + (overall.y2 - overall.y1) * (index + 1) / parts,
          } : null;
          const d = line ? `M ${line.x1} ${line.y1} L ${line.x2} ${line.y2}` : '';
          // Geometry follows transforms without rendering the React tree each frame.
          connectors.current[index]?.querySelectorAll('path').forEach(path => path.setAttribute('d', d));
        });
      }
  ...
  return <section className="demo-journey" aria-label="Transfer route">
    ...
    <motion.div ref={root} id="route-nodes" className={`demo-route-map ${expanded ? 'is-expanded' : ''}`} animate={{ height }} transition={{ duration: reduced ? 0 : 0.48, ease: EASE }} data-gap="8" data-expanded={expanded}>
      <svg className="demo-connectors" aria-hidden="true" width="100%" height="100%">
        {nodes.slice(0, -1).map((node, index) => {
          return <g ref={element => { connectors.current[index] = element; }} key={`${node.id}-${nodes[index + 1]?.id}`} data-connector={`${node.id}-${nodes[index + 1]?.id}`}><path className="demo-path-base"/><path className="demo-path-progress" pathLength="1" strokeDasharray="1" strokeDashoffset={1 - connectorProgress(transfer, index)}/></g>;
        })}
      </svg>
      {nodes.map((node, index) => {
        ...
        return <motion.div key={node.id} data-node={node.id} data-state={state} className={`demo-route-node ${index === nodes.length - 1 ? 'is-recipient' : ''}`} aria-hidden={!expanded && !endpoint}
          ...
          <div className="demo-node-label"><strong>{expanded ? node.name : index === 0 ? node.detail.split(' · ')[0] : node.id === 'recipient' ? node.detail.split(' · ')[0] : node.name}</strong><span>{expanded ? status : index === 0 ? 'From' : 'To'}</span></div>
        </motion.div>;
      })}
    </motion.div>
